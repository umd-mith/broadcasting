  ...
  return (
    <div id="visualization" className="page-programs programs">
        <section>
          ...
          <p>Use this visualization to explore the entities (people and organizations) connected to the NAEB, NFCB, WHA, and KUOM radio collections. Each entity is represented by one circle. Select and de-select the checkboxes for any collection(s) in the legend to highlight the entities who contributed to programs in a collection. Check or uncheck "Toggle lines" to adjust the appearance of the base visualization to your preference.</p>
          <p>Entities are positioned according to the number of items to which they contributed in each collection. Translucent color on a circle indicates that the entity it represent contributed to more than one collection. Solid color indicates that the entity contributed to a single collection. Hover over a circle on the visualization to bring up the entity's name and connected collections, and click on the circle to view more information about the entity's connections to other entities on the visualization.</p>
          <p>The circles that are highlighted when an entity is clicked represent the other entity with which the selected entity collaborated in the four radio collections. The sidebar lists those entities, and common materials shared between the two entities. Click on entity names in the sidebar to go to its associated landing page, and click on the name of a shared item to go to the associated record and/or digitized content associated. Collapse the sidebar using the left-pointing arrow at the top of the sidebar.</p>
          <p>The search bar allows you to filter the highlighted nodes of the visualization to match your search query. Use the 'X' on the right side of the search bar to clear the filter on the visualization. Pan and zoom around the visualization using your mouse and scroll wheel.</p>
